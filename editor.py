...
__version__ = "1.0.2"
GITHUB_EXE_URL = "https://api.github.com/repos/ItsJohnnyy02/ScheduleIEditor-Installer/releases/latest"  # Update with your release URL
CURRENT_EXE = Path(sys.executable).resolve()

def check_for_updates():
    try:
        if getattr(sys, 'frozen', False):  # .exe mode
            GITHUB_API_URL = "https://api.github.com/repos/ItsJohnnyy02/Schedule1SaveEditor/releases/latest"
            logging.debug(f"Checking .exe updates via API: {GITHUB_API_URL}")
            headers = {"User-Agent": "ScheduleIEditor/1.0.0"}
            ...
            for asset in release_data["assets"]:
                if asset["name"] == "Update.exe":
                    ...
                    logging.debug(f"Found editor.exe at: {GITHUB_EXE_URL}")
                    break
            else:
                logging.error("editor.exe not found in release assets")
                raise ValueError("editor.exe not found in latest release assets - ensure it’s included in the release")

            CURRENT = Path(sys.executable).resolve()
            if latest_version > __version__:
                if messagebox.askyesno("Update Available", f"New version {latest_version} available (current: {__version__}). Update now?"):
                    logging.debug(f"Downloading update from: {GITHUB_EXE_URL}")
                    response = requests.get(GITHUB_EXE_URL, stream=True, timeout=10)
                    response.raise_for_status()
                    new_exe_path = CURRENT.parent / "editor_new.exe"
                    with open(new_exe_path, "wb") as f:
                        for chunk in response.iter_content(chunk_size=8192):
                            f.write(chunk)
                    logging.info(f"Downloaded update to {new_exe_path}")
                    batch_script = CURRENT.parent / "update.bat"
...
""")
                ...
                logging.debug("No newer version available")
                messagebox.showinfo("Up to Date", f"Version {__version__} is the latest.")

        else:  # .py mode
            GITHUB_RAW_URL = "https://raw.githubusercontent.com/ItsJohnnyy02/Schedule1SaveEditor/main/editor.py"
            logging.debug(f"Checking .py updates from: {GITHUB_RAW_URL}")
            headers = {"User-Agent": "ScheduleIEditor/1.0.0"}
            response = requests.get(GITHUB_RAW_URL, headers=headers, timeout=5)
            logging.debug(f"Raw file response status: {response.status_code}")
            response.raise_for_status()
            ...
            for line in latest_code.splitlines():
                if line.startswith("__version__ ="):
                    ...
                    logging.debug(f"Latest version from source: {latest_version}")
                    break
            else:
                logging.error("Version not found in source code")
                raise ValueError("Version not found in editor.py")

            if latest_version > __version__:
                if messagebox.askyesno("Update Available", f"New version {latest_version} available (current: {__version__}). Update now?"):
                    logging.debug(f"Downloading updated source from: {GITHUB_RAW_URL}")
                    with open(CURRENT_SCRIPT, "w", encoding="utf-8") as f:
                        f.write(latest_code)
                    logging.info(f"Updated source code at {CURRENT_SCRIPT}")
        ...
        messagebox.showwarning("Update Check Failed", f"Couldn’t check for updates: {str(e)}")
...
class SaveManager:
    ...
    def _download_and_extract_rar(self, rar_name: str, target_dir: Path) -> tuple[Path, None]:
        try:
            editor_dir = Path(sys.executable).parent if getattr(sys, 'frozen', False) else Path(__file__).parent
            data_dir = editor_dir / "data"
            try:
                data_dir.mkdir(exist_ok=True)
            except PermissionError:
                data_dir = Path.home() / "AppData" / "Local" / "ScheduleIEditor" / "data"
            ...
            if not rar_path.exists():
                ...
                if not rar_path.exists():
                    ...
                    raise RuntimeError(f"Failed to download {rar_name}")
                logging.debug(f"Downloaded {rar_name} successfully")
            else:
                logging.debug(f"RAR file {rar_name} already exists")
            ...
            if not extractor:
                raise FileNotFoundError("Neither 7-Zip nor WinRAR found. Install one.")

            if extractor == "7zip":
                logging.debug(f"Extracting with 7-Zip: {seven_zip_path}")
                ...
                logging.debug(f"Extracting with WinRAR: {winrar_path}")
                result = subprocess.run(
            ...
            for item in extracted_contents:
                if item.is_dir() and item.name.lower() == expected_name:
                    extracted_dir = item
                    break
                elif item.is_dir() and item.name.lower() == "properties":
                    extracted_dir = item
                    break
...
class SaveEditor:
    def __init__(self, root):
        self.root = root
        self.root.title(f"Schedule I Save Editor - v{__version__} - © 2025 Johnny")
